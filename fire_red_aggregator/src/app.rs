...
//! tracker server instances simultaneously and renders each player's party,
//! badge progress, and encounter data in its own column. It is designed for
//! Soul Link runs, where two or more players' pokemon are linked by their
//! shared met-location.
//!
//! # Soul Link detection
//!
...
//! relies on met location being unique per encounter area, which holds for
//! standard FireRed but may produce false positives on heavily modified ROMs.
//!
//! # Texture pipeline
//!
...
/// Prevents columns from becoming too narrow to read on small windows.
...
///
/// Owns the list of [`MonitorSlot`]s (one per connected server) and the shared
...
impl AggregatorApp {
    ...
    /// the GPU, then enqueues requests for any species not yet cached.
    ///
    /// Called once per frame before any drawing occurs so textures are always
    /// available when the draw methods run.
    ///
    /// For each slot this method:
    /// 1. Drains `pending_textures` and calls [`egui::Context::load_texture`] for each.
    /// 2. Walks the current party and encounter lists, collecting species IDs not
    ///    yet in `known_species`.
    /// 3. Pushes a deduplicated batch into `texture_request_queue` for the network
    ///    thread to send to the server.
    ///
    /// Species 0 and species > 386 are always skipped as sentinel / out-of-range
    /// values. The server always sends both normal and shiny variants for each
    /// requested species, so only one request per species is needed.
        ...
        for slot in &self.slots {
            ...
            // ── Request any species we don't have yet ────────────────────────
            {
                ...
                for p in &gs.party {
                    ...
                    if species > 0 && species <= 386 && !known.contains(&species) {
                        needed.push(species);
                    }
                }
    ...
    /// Draws a single player's column: badge summary, party members, and a
    /// collapsible encounter section.
    ///
    ...
    fn draw_player_column(
        ...
        if let Some(badge_state) = &gs.badge_state {
            ui.horizontal(|ui| {
                ...
                // Colored dot per badge: green = obtained, grey = not yet.
    ...
    /// Draws a single party member row: optional Soul Link label, sprite,
    /// nickname, shiny indicator, level, and HP.
    ///
    /// # Soul Link detection
    ///
    /// Checks whether the pokemon's `met_location` matches any pokemon in
    /// another player's party. If a match is found a purple label is shown
    /// above the sprite identifying the linked partner and which player owns it.
    ///
    /// # HP color coding
    ///
    ...
    ///
    /// Skips the section entirely if `list` contains no valid species. Each
    /// sprite shows a tooltip with the encounter's min and max level range
    /// when hovered.
    ///
    /// # Arguments
    ///
    /// * `ui`      — The egui [`Ui`] to render into.
    /// * `heading` — Section label shown above the sprite row (e.g. `"Land"`).
...
impl eframe::App for AggregatorApp {
    ...
    /// then renders a responsive multi-column layout — one column per player.
    ///
    /// # Column count
    ///
    /// The number of columns is determined by dividing the available window
    /// width by [`MIN_COLUMN_WIDTH`], clamped to `[1, slot_count]`. The layout
...
///
/// Keys follow the format `"pokemon_{species}_{normal|shiny}"` and are used
