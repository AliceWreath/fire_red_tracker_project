...
impl AggregatorApp {
        ...
        for slot in &self.slots {
            ...
            }
            {
                ...
                for p in &gs.party {
                    ...
                    if s > 0 && s <= 386 && !known.contains(&s) { needed.push(s); }
                }
                for wild in gs.encounters.land_mon_encounters.wild_pokemon_list.iter()
    ...
    fn draw_column(
        ...
        // ── Party region (top) ────────────────────────────────────────────────
        let party_rect = egui::Rect::from_min_size(full_rect.min, egui::vec2(full_rect.width(), party_height));
        ...
        let enc_rect = egui::Rect::from_min_size(enc_min, egui::vec2(full_rect.width(), enc_height));
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                for (idx, pokemon) in gs.party.iter().enumerate() {
                    let others: Vec<_> = all_states.iter()
                        .filter(|(l, _)| l != label)
                        .cloned()
                        .collect();
    ...
    fn draw_encounter_region(
        ...
        Self::draw_encounter_section(ui, "Land",
            &gs.encounters.land_mon_encounters.wild_pokemon_list, textures);
        Self::draw_encounter_section(ui, "Water / Fishing",
            &gs.encounters.water_mon_encounters.wild_pokemon_list
                ...
                .collect::<Vec<_>>(),
            textures);
        Self::draw_encounter_section(ui, "Rock Smash",
            &gs.encounters.rock_smash_encounters.wild_pokemon_list, textures);
    }
                ...
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new(pokemon.get_nickname_string())
                            .strong().size(16.0).color(egui::Color32::WHITE),
                    );
                    if shiny { ui.label(egui::RichText::new("✨").size(14.0)); }
                    ui.label(format!("Lv{}", pokemon.level));
                });
                let hp_ratio = pokemon.hp as f32 / pokemon.max_hp as f32;
                let hp_color = if hp_ratio < 0.3 { egui::Color32::RED }
                    else if hp_ratio < 0.8 { egui::Color32::YELLOW }
                    else { egui::Color32::WHITE };
                ui.label(
                    egui::RichText::new(format!("{}/{}", pokemon.hp, pokemon.max_hp))
                        .color(hp_color).size(14.0),
                );
